...
final public class PredatorClock {
    ...
    public func draw(in rect: CGRect, context: CGContext) {
        ...
        if (self.configuration.showSeconds) {
            let width = min(rect.size.width, rect.size.height) * 0.211
            let height = width * 1.57
            let margins = CGPoint(x: width * 0.037, y: width * 0.037)
            let center = CGPoint(x: rect.width / 2.0, y: rect.height / 2.0)
            let nummberSize = CGSize(width: width, height: height)
            // Hours
            drawHourHightNumber(in: CGRect(
                x: center.x - nummberSize.width * 2.0 - margins.x,
                y: center.y + margins.y,
                width: nummberSize.width,
                height: nummberSize.height
            ), context: context, value: Int(hours / 10))
            drawHourLowNumber(in: CGRect(
                x: center.x - nummberSize.width - margins.x,
                y: center.y + margins.y,
                width: nummberSize.width,
                height: nummberSize.height
            ), context: context, value: Int(hours % 10))
            ...
            // Seconds
            drawSecondHightNumber(in: CGRect(
                x: center.x - nummberSize.width,
                y: center.y - nummberSize.height - margins.y,
                width: nummberSize.width,
                height: nummberSize.height
            ), context: context, value: Int(seconds / 10))
            drawSecondLowNumber(in: CGRect(
                x: center.x,
                y: center.y - nummberSize.height - margins.y,
                width: nummberSize.width,
                height: nummberSize.height
            ...
            let width = min(rect.size.width, rect.size.height) * 0.257
    ...
    func drawSecondHightNumber(in rect: CGRect, context: CGContext, value: Int) {
        let width = rect.size.width * 0.384
        let height = width * 0.2
        let shape = CGSize(width: width, height: height)
        let center = CGPoint(x: rect.origin.x + rect.width / 2.0, y: rect.origin.y + rect.height / 2.0)
    ...
    func drawSecondLowNumber(in rect: CGRect, context: CGContext, value: Int) {
        let width = rect.size.width * 0.384
        let height = width * 0.2
        let shape = CGSize(width: width, height: height)
        let center = CGPoint(x: rect.origin.x + rect.width / 2.0, y: rect.origin.y + rect.height / 2.0)
